<?php
class NewsHubMikadoLike {

	private static $instance;

	private function __construct() {
		add_action('wp_enqueue_scripts', array( $this, 'enqueue_scripts'));
		add_action('wp_ajax_newshub_mikado_like', array( $this, 'ajax'));
		add_action('wp_ajax_nopriv_newshub_mikado_like', array( $this, 'ajax'));
	}
	...
	public function add_like() {
		...
		$title = esc_html__('Like this', 'newshub');
		if( isset($_COOKIE['mkd-like_'. $post->ID]) ){
			$class = 'mkd-like liked';
			$title = esc_html__('You already like this!', 'newshub');
		}
...
if ( !function_exists( 'newshub_mikado_create_like' ) ) {

    function newshub_mikado_create_like() {
        NewsHubMikadoLike::get_instance();
    }

    add_action('after_setup_theme', 'newshub_mikado_create_like');
}