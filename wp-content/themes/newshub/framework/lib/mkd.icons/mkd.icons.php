...
/*
  Class: NewsHubMikadoIconCollections
...
class NewsHubMikadoIconCollections
{
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new NewsHubMikadoIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new NewsHubMikadoIconsFontElegant("Font Elegant", "fe_icon"));
        $this->addIconCollection('ion_icons', new NewsHubMikadoIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linea_icons', new NewsHubMikadoLineaIcons('Linea Icons', 'linea_icon'));
        $this->addIconCollection('simple_line_icons', new NewsHubMikadoSimpleLineIcons('Simple Line Icons', 'simple_line_icons'));
    ...
    public function getVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false) {
        ...
        $iconPackParams = array(
            'type' => 'dropdown',
            'heading' => esc_html__('Icon pack','newshub'),
        ...
        if (is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach ($this->iconCollections as $key => $collection) {
                $iconSetParams[] = array(
                    'type' => 'dropdown',
                    'heading' => esc_html__('Icon','newshub'),
    ...
    public function getSocialIconWidgetParamsArray() {
        ...
        $iconPackParams[] = array(
            'type' => 'dropdown',
            'title' => esc_html__('Icon Pack','newshub'),
            'name' => 'icon_pack',
            'options' => array(
            ...
            'description' => esc_html__('Choose a social icon pack and then select an icon from that pack which you would like to display','newshub')
        );
    ...
    public function getIconCollectionIcons(iNewsHubMikadoIconCollection $collection) {
...
if ( ! function_exists( 'newshub_mikado_activate_theme_icons' ) ) {

    function newshub_mikado_activate_theme_icons() {
        global $newshub_IconCollections;
        $newshub_IconCollections = NewsHubMikadoIconCollections::get_instance();
    }

    add_action('after_setup_theme', 'newshub_mikado_activate_theme_icons');
}