...
class NewsHubMikadoFramework {
    ...
    private function __construct() {
        $this->mkdOptions = NewsHubMikadoOptions::get_instance();
        $this->mkdMetaBoxes = NewsHubMikadoMetaBoxes::get_instance();
    }
	...
	public function setSkin(NewsHubMikadoSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class NewsHubMikadoSkinManager
 *
...
/**
 * Class NewsHubMikadoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class NewsHubMikadoSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo newshub_mikado_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class NewsHubMikadoOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $newshub_options)) {
            return $newshub_options[$key];
...
/*
   Class: NewsHubMikadoAdminPage
   A class that initializes Mikado Admin Page
*/
class NewsHubMikadoAdminPage implements iNewsHubMikadoLayoutNode {
    ...
    function __construct($slug="", $title="", $icon = "") {
        $this->layout = array();
        $this->factory = new NewsHubMikadoFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iNewsHubMikadoRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: NewsHubMikadoMetaBoxes
...
/*
   Class: NewsHubMikadoMetaBox
   A class that initializes Mikado Meta Box
*/
class NewsHubMikadoMetaBox implements iNewsHubMikadoLayoutNode {
	...
	public function setTitle($label) {
		...
		return $newshub_Framework->getSkin()->getSkinLabel().' '.$label;
 	}
}

if ( ! function_exists( 'newshub_mikado_init_framework_variable' ) ) {

    function newshub_mikado_init_framework_variable() {

        global $newshub_Framework;

        $newshub_Framework = NewsHubMikadoFramework::get_instance();
        $mkdSkinManager = new NewsHubMikadoSkinManager();
        $newshub_Framework->setSkin($mkdSkinManager->getSkin());
    }

    add_action('newshub_mikado_before_options_map', 'newshub_mikado_init_framework_variable' );
}
?>